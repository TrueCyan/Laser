...
public class LevelEditor : EditorWindow
{
	...
	private GameObject InstantiateTile(Vector2 pos, Layer layer)
	{
        ...
        var blockColor = metaTile.GetComponent<BlockColor>();
        if (blockColor)
        {
            var gizmoColor = GizmoTile.GetComponent<BlockColor>();
    ...
    private StageBorder.Blocked ConvertDir(Vector2 dir)
    {
        if (dir == new Vector2(1, 0))
        {
            return StageBorder.Blocked.Left;
        }
        if (dir == new Vector2(1, 1))
        {
            return StageBorder.Blocked.LeftDown;
        }
        if (dir == new Vector2(0, 1))
        {
            return StageBorder.Blocked.Down;
        }
        if (dir == new Vector2(-1, 1))
        {
            return StageBorder.Blocked.DownRight;
        }
        if (dir == new Vector2(-1, 0))
        {
            return StageBorder.Blocked.Right;
        }
        if (dir == new Vector2(-1, -1))
        {
            return StageBorder.Blocked.RightUp;
        }
        if (dir == new Vector2(0, -1))
        {
            return StageBorder.Blocked.Up;
        }
        if (dir == new Vector2(1, -1))
        {
            return StageBorder.Blocked.UpLeft;
        }
	...
	private void InstantiateBorder(GameObject borderPrefab, Vector2 pos, Layer layer)
    {
        ...
        if (borderPrefab == null || mouseOverWindow.ToString() != " (UnityEditor.SceneView)")
            return;
        if (borderPrefab.GetComponent<StageBorder>() == null) return;
        ...
        foreach (var dir in directions)
        {
            ...
            if (prevTile != null && prevTile.GetComponent<StageBorder>())
            {
                ...
                Undo.RegisterCompleteObjectUndo(sr, "UpdatedBorder");
				border.blockedFaces |= ConvertDir(dir);
                border.UpdateSprite();
    ...
    private void FixBorder(GameObject borderPrefab, Vector2 pos, Layer layer)
    {
        ...
        foreach (var dir in directions)
        {
            ...
            if (prevTile != null && prevTile.GetComponent<StageBorder>())
            {
                ...
                border.blockedFaces &= ~ConvertDir(dir);
                border.UpdateSprite();
                if (border.blockedFaces == StageBorder.Blocked.None)
                {
                    Undo.DestroyObjectImmediate(prevTile);
                }
            }
            else if(prevTile != null)
            {
                hereBlocked |= ConvertDir(-dir);
            }
        }

        if (hereBlocked != StageBorder.Blocked.None)
        {
            GameObject metaTile = (GameObject)PrefabUtility.InstantiatePrefab(borderPrefab);
            ...
            var border = metaTile.GetComponent<StageBorder>();
            border.blockedFaces = hereBlocked;
            border.UpdateSprite();
            Undo.RegisterCreatedObjectUndo(metaTile, "CreatedTile");
		}
    }
	...
	void OnGUI()
	{
        ...
        bool[] colorMode = {false, false, false};

        using (new EditorGUILayout.HorizontalScope())
        {
            using (new EditorGUI.DisabledGroupScope(!CurPrefab || !CheckHasColor(CurPrefab)))
            {
				EditorGUILayout.PrefixLabel(new GUIContent("Color", "배치할 오브젝트의 색"));
                colorMode[0] = GUILayout.Toggle(RedOn, "Red", EditorStyles.miniButtonLeft);
                colorMode[1] = GUILayout.Toggle(GreenOn, "Green", EditorStyles.miniButtonMid);
                colorMode[2] = GUILayout.Toggle(BlueOn, "Blue", EditorStyles.miniButtonRight);
            }
        }
